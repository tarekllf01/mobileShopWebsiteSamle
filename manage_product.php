...
if(isset($_GET["del"])){
	$id=$_GET["del"];
	$id=(int)$id;
	$del=mysql_query("SELECT image FROM mobile WHERE id='$id' ");
		$row=mysql_fetch_array($del);
		$image=$row["image"];
	$delete=mysql_query("DELETE  FROM mobile WHERE id='$id' ");
	if($delete){
...
<ul>
			...
			<ul >
		    	...
		    	<div class="about">
					...
					$product=htmlentities($product);
						$title="%".$title."%";
					$query=mysql_query("SELECT * FROM mobile WHERE title LIKE '$title'  LIMIT 0,$limit ");
					...
					$query=mysql_query("SELECT * FROM mobile ORDER BY id DESC LIMIT $start,$limit ");
					$query1=mysql_query("SELECT * FROM mobile");
    ...
    <a class="button" href="manage_product.php?del=<?php echo $row["id"]; ?>" role="button">
	<span>remove</span>
	<div class="icon">
    ...
    <script src='http://cdnjs.cloudflare.com/ajax/libs/jquery/2.1.3/jquery.min.js'></script>
        ...
        <script src="button/js/index.js"></script>
					...
					<?php
					if(($next-1) >0){
						echo "<a href=\"manage_product.php?next=".($next-1) . "\" > PREV </a> ";
					}
					if($total > (($next+1)*$limit )){
						echo "<a href=\"manage_product.php?next=" .($next+1 ). "\" > NEXT </a> ";
